...
const TMP_DIR = path.join(ROOT, "work", "presentations", "handmade-revision", "tmp");
const PREVIEW_DIR = path.join(TMP_DIR, "preview");
const LAYOUT_DIR = path.join(TMP_DIR, "layout");
const QA_DIR = path.join(TMP_DIR, "qa");
const FINAL_PPTX = path.join(OUT_DIR, "handmade_revised_scientific_phd_defense.pptx");
...
{
  const s = newSlide(); slideNo++;
  addTitle(s, "LRT analysis", "The LRT asks whether genes change anywhere across the acute time course", "For trajectory visualization, row-scaled VST expression is the most defensible y-axis.");
  addShape(s, { left: 102, top: 220, width: 330, height: 250 }, colors.white, colors.line);
  addText(s, "Statistical test", { left: 134, top: 254, width: 266, height: 30 }, { size: 24, bold: true, color: colors.navy, alignment: "center" });
  addText(s, "Full model:\n~ replicate + timepoint\n\nReduced model:\n~ replicate", { left: 142, top: 318, width: 250, height: 112 }, { size: 19, color: colors.ink, alignment: "center" });
  addShape(s, { left: 474, top: 220, width: 330, height: 250 }, colors.paleGreen, colors.line);
  addText(s, "Visualization y-axis", { left: 506, top: 254, width: 266, height: 30 }, { size: 24, bold: true, color: colors.teal, alignment: "center" });
  addText(s, "Z-score of VST-normalized expression\n\nAlso label as:\nrow-scaled VST expression", { left: 510, top: 318, width: 258, height: 112 }, { size: 20, bold: true, color: colors.ink, alignment: "center" });
  addShape(s, { left: 846, top: 220, width: 330, height: 250 }, colors.white, colors.line);
  addText(s, "Why not log2FC?", { left: 878, top: 254, width: 266, height: 30 }, { size: 24, bold: true, color: colors.accent, alignment: "center" });
  addText(s, "LRT is an overall time-effect test, not a single contrast. Z-score preserves the temporal pattern across Vehicle, 5 min and 15 min.", { left: 884, top: 324, width: 254, height: 104 }, { size: 18, color: colors.ink, alignment: "center" });
  addKpi(s, "19", "LRT-significant time-varying genes", { left: 486, top: 520, width: 308, height: 118 }, colors.teal);
  addFooter(s, slideNo, "Source: DESeq2 LRT; trajectory plotted as VST z-score");
  addNotes(s, [
    "Use this slide to explain what the LRT tests before showing the trajectory.",
    "The y-axis choice matters: row-scaled VST expression shows shape, whereas log2FC would impose a pairwise interpretation."
  ]);
}
...
{
  const s = newSlide(); slideNo++;
  addTitle(s, "LRT trajectories", "Nineteen LRT genes show coordinated time-dependent expression patterns", "The plot emphasizes temporal shape rather than absolute gene abundance.");
  await addImage(s, assets.lrt19, { left: 74, top: 184, width: 850, height: 454 }, "LRT 19 gene trajectory with VST z-score y-axis");
  addShape(s, { left: 960, top: 242, width: 214, height: 276 }, colors.paleGreen, colors.line);
  addText(s, "Readout", { left: 986, top: 276, width: 162, height: 30 }, { size: 23, bold: true, color: colors.teal, alignment: "center" });
  addBullets(s, [
    "Rows are genes; values are scaled per gene.",
    "Patterns converge on a 5-min pulse and recovery/rebound.",
    "This supports temporal coordination beyond pairwise DEGs."
  ], { left: 988, top: 326, width: 162, height: 132 }, { size: 15.5 });
  addFooter(s, slideNo, "Source: results/objects/LRT_19_trajectory_VSTzscore_v1.png");
  addNotes(s, [
    "Point out that this figure uses Z-score of VST-normalized expression.",
    "Describe the plot as time-dependent genes identified by LRT, not as a Wald contrast."
  ]);
}
